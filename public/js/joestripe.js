...
//   $mycartBadge.text(joesCart.products.length);
  ...
  //  $("#cart-button").text("Cart (" + this.cartQtyTotalNew + ")")
    ...
    // if in test card mode make sure card number = 4242424242424242
    if (joesTestCharges && ($("#card_card")[0].value != "4242424242424242")) {
          errorText = "<p> using test charges Card Number must be 4242424242424242</p>"
             $("#result").append(errorText)
            $(parent).addClass("has-error")
            valid = false;
    }
